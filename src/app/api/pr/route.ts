...
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      pullRequests: savedPullRequests,
    });
  } catch (error) {
...
export async function POST(req: Request) {
  ...
  try {
    const existingPRs = await prisma.pullRequest.findMany({
      where: {
        prNumber: {
          in: pullRequestsData.map((pr: PullRequest) => pr.prNumber),
        },
        //@ts-ignore
        userId: session?.user?.id,
      },
    ...
    const newPullRequestsData = pullRequestsData.filter(
      (prData: PullRequest) => {
        return !existingPRs.some(
          (existingPR) =>
            existingPR.prNumber === prData.prNumber &&
            existingPR.repoURL === prData.repoURL,
        );
      },
    );
    ...
    if (newPullRequestsData.length > 0) {
      ...
      return NextResponse.json({
        success: false,
        message: 'No new pull requests to create',
      });
    }
  } catch (error) {
    console.error('Error creating pull requests:', error);
...
export async function DELETE(req: Request) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Pull Request not found or you don't have permission to delete it",
        },
        { status: 404 },
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'Pull Request has been deleted',
    });
  } catch (error) {
    console.error('Error creating pull requests:', error);
    return NextResponse.json({
      success: false,
      message: 'Failed to delete pull requests',
    });
  }
}
