...
export async function GET(req: Request) {
    try {
        ...
        const leaderBoardWithTotalPoints = leaderboard.map(user => ({
            ...
            bounties: user.pullRequests.reduce((acc, pr) => acc + (pr?.bounty ? pr?.bounty : 0), 0)
        }))

        leaderBoardWithTotalPoints.sort((a, b) => b.totalPoints - a.totalPoints)
        return NextResponse.json({ success: true, data: leaderBoardWithTotalPoints, message: "Leaderboard sorted and fetched successfully" });
