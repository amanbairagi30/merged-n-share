...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existingView) {
      ...
      return NextResponse.json(
        { message: 'View updated successfully', isNewView: false },
        { status: 200 },
      );
    } else {
      // Create a new view
      ...
      return NextResponse.json(
        { message: 'New view recorded successfully', isNewView: true },
        { status: 200 },
      );
    }
  } catch (error) {
